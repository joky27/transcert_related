...
def update_cert_2(chain, ca_cert, ca_private_key, fconf):
    ...
    if which_operation == 0:#delete extension
        ...
        extension_type = random.choice(extension_json.keys())
...
def on_the_fly_testing(fileid):
    os.system("sh scripts/verify_openssl_polarssl_gnutls_matrixssl.sh "+str(fileid)+'.pem')
    open_path = os.path.join(conf.test_results_path,str(fileid)+'.txt')
